...
export default function VideoPage({ video, relatedVideos, hasAccess }: VideoPageProps) {
  const router = useRouter();
  const { data: session } = useSession();

  // Share handlers
  const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
  const shareTitle = decodeHtmlEntities(video?.title?.rendered || '');
  ...
  if (router.isFallback || !video) {
    return <Layout><div className={styles.loading}>Loading...</div></Layout>;
  }

  const category = video._embedded?.['wp:term']?.[0]?.[0];
  const author = video._embedded?.author?.[0];
  const featuredImage = video._embedded?.['wp:featuredmedia']?.[0];

  // Check if this is premium content and user doesn't have access
  const isPremium = video.isPremium || video.meta?.isPremium || false;
  const requiredTier = (video.requiredTier || video.meta?.requiredTier || 'collective') as 'collective' | 'insider';

  if (isPremium && !hasAccess) {
    return (
      <Layout>
        <SEO
          title={decodeHtmlEntities(video.title.rendered)}
          description={decodeHtmlEntities(video.excerpt?.rendered?.replace(/<[^>]*>/g, '') || '')}
          url={`https://www.success.com/video/${video.slug}`}
          type="video"
          image={featuredImage?.source_url}
        />
        <Paywall
          requiredTier={requiredTier}
          articleTitle={decodeHtmlEntities(video.title.rendered)}
          excerpt={decodeHtmlContent(video.excerpt?.rendered || '')}
        />
      </Layout>
    );
  }
  ...
  // Format date
  const videoDate = new Date(video.date).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
  ...
  const seoDescription = video.excerpt?.rendered
    ? getPlainText(video.excerpt.rendered)
    : getPlainText(video.content?.rendered?.substring(0, 300) || '');

  // Structured data for video
  const structuredData = {
    '@context': 'https://schema.org',
    '@type': 'VideoObject',
    name: decodeHtmlEntities(video.title.rendered),
    description: seoDescription,
    thumbnailUrl: featuredImage?.source_url,
      ...
      <article className={styles.article}>
        ...
        <header className={styles.header}>
          <div className={styles.headerContent}>
            ...
            <h1 className={styles.title}>{decodeHtmlEntities(video.title.rendered)}</h1>
            ...
            {video.excerpt?.rendered && (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: decodeHtmlContent(video.excerpt.rendered) }}
              />
            )}
          </div>
        </header>
        ...
        <div className={styles.actionsSection}>
          <div className={styles.shareSection}>
            <h3 className={styles.shareTitle}>Share This Video</h3>
          ...
          <div className={styles.relatedContainer}>
            ...
            <div className={styles.relatedGrid}>
              {relatedVideos.map((relatedVideo) => {
                const relatedImage = relatedVideo._embedded?.['wp:featuredmedia']?.[0];
                const relatedCategory = relatedVideo._embedded?.['wp:term']?.[0]?.[0];

                return (
                  <a key={relatedVideo.id} href={`/video/${relatedVideo.slug}`} className={styles.relatedCard}>
                    {relatedImage && (
                      <img
                        src={relatedImage.source_url}
                        alt={decodeHtmlEntities(relatedVideo.title.rendered)}
                    ...
                    <div className={styles.relatedContent}>
                      ...
                      <h3 className={styles.relatedCardTitle}>{decodeHtmlEntities(relatedVideo.title.rendered)}</h3>
                    </div>
                  </a>
                );
              })}
...
export async function getServerSideProps({ params, req, res }: any) {
  ...
  try {
    const videos = await fetchWordPressData(`videos?slug=${slug}&_embed`);
    ...
    const categoryId = video._embedded?.['wp:term']?.[0]?.[0]?.id;
    let relatedVideos = [];

    if (categoryId) {
      const related = await fetchWordPressData(
        `videos?categories=${categoryId}&_embed&per_page=3&exclude=${video.id}`
      );
    ...
    const { authOptions } = await import('../../api/auth/[...nextauth].ts');
    const session = await getServerSession(req, res, authOptions);

    const isPremium = video.isPremium || video.meta?.isPremium || false;
    const requiredTier = video.requiredTier || video.meta?.requiredTier || 'collective';
    ...
    console.error(`[Video] Error fetching video "${slug}":`, error);
    return { notFound: true };
  }
}
