...
async function main() {
  console.log('📦 Setting up store products...\n');

  const supabase = createClient(supabaseUrl, supabaseKey);

  // Step 1: Check if table exists, if not show SQL to run
  ...
  if (checkError && checkError.message.includes('does not exist')) {
    console.log('⚠️  Table store_products does not exist yet.\n');
    console.log('Please run this SQL in your Supabase SQL Editor:\n');
    console.log('----------------------------------------\n');
    console.log(`-- Create store_products table
...
CREATE POLICY "Allow public read" ON store_products FOR SELECT USING (is_active = true);
CREATE POLICY "Allow admin all" ON store_products FOR ALL TO authenticated USING (true);
`);
    console.log('----------------------------------------\n');
  ...
  console.log(`📥 Importing ${storeProducts.length} products...\n`);
  ...
  console.log('\n🎉 All done! Products are ready in /admin/store-products\n');
}
